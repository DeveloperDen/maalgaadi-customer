...
export default class PastMyBookings extends Component {
  ...
  getPastBookings = async () => {
    ...
    const request = await fetch(
      Constants.BASE_URL + Constants.GET_COMPLETED_TRIP,
      {
        method: 'POST',
        body: reqBody,
        headers: {
  ...
  render() {
    return (
      <View style={{flex: 1}}>
        <ScrollView
          style={{
            display: this.state.bookings.length > 0 ? 'flex' : 'none',
            opacity: this.state.gettingPages ? 0.05 : 1,
          }}
          ...
          {this.state.bookings.map((value, index) => {
            return (
              <TouchableHighlight
                ...
                onPress={async () => {
                  await DataController.setItem(
                    DataController.COMPLETED_TRIP_DATA,
                  ...
                  <View
                    ...
                    style={{flexDirection: 'row'}}>
                      <Text
                        style={{
                          backgroundColor:
                            value.status === COMPLETED ? GREEN : 'red',
                          paddingVertical: 3,
                      ...
                      {value.pod_charge > 0 && value.status === COMPLETED ? (
                        <Text
                          style={{
                            backgroundColor: GREEN,
                            paddingVertical: 3,
        ...
        <View
          ...
          <Text style={{textAlign: 'center', fontSize: 20}}>
            {this.state.isLoading
              ? 'Getting your Past Bookings...'
              : Constants.PAST_BOOK_EMPTY}
          </Text>
        </View>
